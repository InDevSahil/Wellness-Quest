...
function scheduleDailyReminder() {
    ...
    // Check every minute if it's time to show reminder
    notificationInterval = setInterval(() => {
        const now = new Date();
        const currentTime = now.getHours() * 100 + now.getMinutes();
        const reminderTime = parseInt(state.reminders.time.replace(':', ''));

        if (currentTime === reminderTime) {
            showNotification(
                'Wellness Quest Reminder',
                'Time for your daily wellness check-in! 🌟',
                'data:image/svg+xml;base64,'
            );
        }
...
function updateReminderSettings() {
    ...
    if (enabled) {
        ...
        document.getElementById('notificationStatus').textContent = `Reminders enabled for ${time}`;
    } else {
        clearInterval(notificationInterval);
        notificationInterval = null;
        document.getElementById('notificationStatus').textContent = 'Reminders disabled';
    }
}

function loadReminderSettings() {
    document.getElementById('enableReminder').checked = state.reminders.enabled;
    document.getElementById('reminderTime').value = state.reminders.time;
    document.getElementById('notificationStatus').textContent = state.reminders.enabled ?
        `Reminders enabled for ${state.reminders.time}` : 'Reminders disabled';
}

async function initializeReminders() {
    const permissionGranted = await requestNotificationPermission();
    if (!permissionGranted) {
        document.getElementById('notificationStatus').textContent = 'Notifications not supported or denied';
        document.getElementById('enableReminder').disabled = true;
        return;
    }
...
// Step counter variables for walk10 quest
let stepCount = 0;
let stepCounterActive = false;
let lastAcceleration = { x: null, y: null, z: null };
let stepThreshold = 1.2; // Threshold for step detection (tweak as needed)
...
function startStepCounter() {
    if (typeof DeviceMotionEvent !== 'undefined' && typeof DeviceMotionEvent.requestPermission === 'function') {
        ...
        DeviceMotionEvent.requestPermission()
            .then(response => {
                if (response === 'granted') {
                    ...
                    showToast('Permission denied for motion sensors. Step counter disabled.');
                }
            })
            .catch(() => {
                showToast('Motion sensor permission request failed. Step counter disabled.');
            });
        ...
        showToast('Device motion not supported. Step counter disabled.');
    }
}
...
function startTaskTimer(durationMinutes) {
    ...
    // If the active task is walk10, start step counter
    if (activeTask && activeTask.id === 'walk10') {
        startStepCounter();
    }
    ...
    taskTimer = setInterval(() => {
        ...
        if (remaining <= 0) {
            ...
            // Stop step counter when timer ends
            if (activeTask && activeTask.id === 'walk10') {
                stopStepCounter();
            }
        }
...
function updateTaskTimerUI(remainingSeconds) {
    ...
    // Update step count UI if active task is walk10
    if (activeTask && activeTask.id === 'walk10') {
        updateStepCountUI();
    }
}
...
function submitTaskProof(proofText) {
    ...
    // If active task is walk10, stop step counter
    if (activeTask && activeTask.id === 'walk10') {
        stopStepCounter();
        showToast(`You took ${stepCount} steps during your walk!`);
    }
...
window.onload = function() {
    ...
    document.getElementById('enableReminder').addEventListener('change', updateReminderSettings);
